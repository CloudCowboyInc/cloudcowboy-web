...
export default function AccessBoard() {
  ...
  const approve = async (r: AccessRequest, role: "investor" | "admin") => {
    if (!supabase) return;
    const e1 = await supabase.from("allowlist").upsert({ email: r.email.toLowerCase(), role, approved_by: adminEmail });
    const e2 = await supabase.from("access_requests").update({ status: "approved" }).eq("id", r.id);
  ...
  const deny = async (r: AccessRequest) => {
    if (!supabase) return;
    const { error } = await supabase.from("access_requests").update({ status: "denied" }).eq("id", r.id);
  ...
  return (
    <div className="space-y-6">
            ...
            {pending.map((r) => (
              <Card key={r.id} className="flex flex-wrap items-center justify-between gap-3 border-border/60 bg-card/60 p-4">
                <div className="text-sm">
                  ...
                  <div className="text-muted-foreground">{[r.name, r.org].filter(Boolean).join(" · ") || "—"}</div>
      ...
      <div>
        <h2 className="mb-3 font-display text-lg">Approved members ({allow.length})</h2>
        <div className="space-y-2">
          {allow.map((row) => (
            <div key={row.email} className="flex items-center justify-between rounded-md border border-border/60 bg-card/40 px-4 py-2 text-sm">
              ...
              <Button size="sm" variant="ghost" className="text-destructive" onClick={() => revoke(row)}>Revoke</Button>
            </div>
          ))}
        </div>
      </div>
